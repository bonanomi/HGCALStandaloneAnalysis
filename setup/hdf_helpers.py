...
def esum_to_hdf(energy, isInj=False, isMC=False, dEdx = False, outdir = './'):
    if isMC:
        hdf_name = 'esum_mc_%i.h5' %energy
        df = pd.read_hdf(path_hdf+'mc_%i.h5' %energy, 'df')
    else:
        if isInj:
            hdf_name = 'esum_data_inj_%i.h5' %energy
            df = pd.read_hdf(path_hdf+'data_inj_%i.h5' %energy, 'df')
        else:
...
def cog_to_hdf(energy, isInj=False, isMC=False, outdir = './'):
    if isMC:
        hdf_name = 'cog_mc_%i.h5' %energy
        df = pd.read_hdf(path_hdf+'mc_%i.h5' %energy, 'df')
    else:
        if isInj:
            hdf_name = 'cog_data_inj_%i.h5' %energy
            df = pd.read_hdf(path_hdf+'data_inj_%i.h5' %energy, 'df')
        else:
...
def lp_to_hdf(energy, isInj=False, isMC=False, dEdx = False, outdir = './'):
    if isMC:
        hdf_name = 'lp_mc_%i.h5' %energy
        df = pd.read_hdf(path_hdf+'mc_%i.h5' %energy, 'df')
    else:
        if isInj:
            hdf_name = 'lp_data_inj_%i.h5' %energy
            df = pd.read_hdf(path_hdf+'data_inj_%i.h5' %energy, 'df')
        else:
            hdf_name = 'lp_data_%i.h5' %energy
            df = pd.read_hdf(path_hdf+'data_%i.h5' %energy, 'df')
